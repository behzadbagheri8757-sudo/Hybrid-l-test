...
async function importBackupJSON(file){
  try{
    ...
    await crmPersistencePut(PRERESTORE_KEY, JSON.stringify(data));
...
async function undoLastRestore(){
  try{
    ...
    try{ await crmPersistenceDelete(PRERESTORE_KEY); }catch(e){ console.error('pre-restore snapshot cleanup failed', e); }
    try{
...
async function autoBackupTick(){
  ...
  await crmPersistencePut(key, JSON.stringify(data));
  list.push({key, ts});
  while(list.length > AUTO_BACKUP_MAX){
    const old = list.shift();
    try{ await crmPersistenceDelete(old.key); }catch(e){ /* نبود یا حذف نشد، مهم نیست */ }
  }
